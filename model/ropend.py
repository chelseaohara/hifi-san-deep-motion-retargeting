from torch import arange, cat, meshgrid, ones_like, polar, view_as_complex, view_as_real
...
class RoPENd(nn.Module):
    ...
    def __init__(self, shape, base=10000):
        ...
        theta_ks = 1 / (base ** (arange(k_max) / k_max))

        # create a stack of angles multiplied by position
        angles = cat([t.unsqueeze(-1) * theta_ks for t in
                            meshgrid([arange(d) for d in channel_dims], indexing='ij')], dim=-1)

        # convert to complex number to allow easy rotation
        rotations = polar(ones_like(angles), angles)
    ...
    def forward(self, x):
        ...
        x = view_as_complex(x.reshape(*x.shape[:-1], -1, 2))
        pe_x = self.rotations * x
        return view_as_real(pe_x).flatten(-2)
